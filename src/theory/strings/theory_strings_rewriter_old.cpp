/*********************                                                        */
/*! \file theory_strings_rewriter_old.cpp
 ** \verbatim
...
Node TheoryStringsRewriterOld::simpleRegexpConsume( std::vector< Node >& mchildren, std::vector< Node >& children, int dir ){
...
Node TheoryStringsRewriterOld::rewriteConcatString( TNode node ) {
...
void TheoryStringsRewriterOld::mergeInto(std::vector<Node> &t, const std::vector<Node> &s) {
...
void TheoryStringsRewriterOld::shrinkConVec(std::vector<Node> &vec) {
  unsigned i = 0;
...
Node TheoryStringsRewriterOld::applyAX( TNode node ) {
...
Node TheoryStringsRewriterOld::prerewriteConcatRegExp( TNode node ) {
...
Node TheoryStringsRewriterOld::prerewriteOrRegExp(TNode node) {
...
Node TheoryStringsRewriterOld::prerewriteAndRegExp(TNode node) {
...
bool TheoryStringsRewriterOld::isConstRegExp( TNode t ) {
...
bool TheoryStringsRewriterOld::testConstStringInRegExp( CVC4::String &s, unsigned int index_start, TNode r ) {
...
Node TheoryStringsRewriterOld::rewriteMembership(TNode node) {
  Node retNode = node;
  Node x = node[0];
...
RewriteResponse TheoryStringsRewriterOld::postRewrite(TNode node) {
...
bool TheoryStringsRewriterOld::hasEpsilonNode(TNode node) {
  for(unsigned int i=0; i<node.getNumChildren(); i++) {
...
RewriteResponse TheoryStringsRewriterOld::preRewrite(TNode node) {
  Node retNode = node;
  Node orig = retNode;
...
Node TheoryStringsRewriterOld::rewriteIndexof( Node node ) {
  std::vector< Node > children;
  getConcat( node[0], children );
...
Node TheoryStringsRewriterOld::rewriteReplace( Node node ) {
  if( node[1]==node[2] ){
    return node[0];
  }else{
    if( node[1].isConst() ){
...
void TheoryStringsRewriterOld::getConcat( Node n, std::vector< Node >& c ) {
...
Node TheoryStringsRewriterOld::mkConcat( Kind k, std::vector< Node >& c ){
...
Node TheoryStringsRewriterOld::splitConstant( Node a, Node b, int& index, bool isRev ) {
...
bool TheoryStringsRewriterOld::canConstantContainConcat( Node c, Node n, int& firstc, int& lastc ) {
  Assert( c.isConst() );
...
bool TheoryStringsRewriterOld::canConstantContainList( Node c, std::vector< Node >& l, int& firstc, int& lastc ) {
  Assert( c.isConst() );
  CVC4::String t = c.getConst<String>();
  //must find constant components in order
  size_t pos = 0;
...
Node TheoryStringsRewriterOld::getNextConstantAt( std::vector< Node >& vec, unsigned& start_index, unsigned& end_index, bool isRev ) {
...
Node TheoryStringsRewriterOld::collectConstantStringAt( std::vector< Node >& vec, unsigned& end_index, bool isRev ) {
  std::vector< Node > c;
