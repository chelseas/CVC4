 ...
 **
 ** \brief Preprocessing pass for solving ints as bit vectors
 ** Preprocessing pass for solving ints as bit vectors.
 **/
...
SolveIntAsBVPass::SolveIntAsBVPass(PreprocessingPassRegistry* registry)
    : PreprocessingPass(registry, "solveIntAsBV", true) {}

PreprocessingPassResult SolveIntAsBVPass::applyInternal(
...
Node SolveIntAsBVPass::intToBVMakeBinary(TNode n, NodeMap& cache) {
  ...
  while (!toVisit.empty()) {
    ...
    if (stackHead.children_added) {
      ...
      if (current.getNumChildren() > 2 && (current.getKind() == kind::PLUS ||
        ...
        switch (newKind) {
          ...
          default:
            ...
            throw TypeCheckingException(
                current.toExpr(),
                std::string("Cannot translate to BV: ") + current.toString());
        }
          ...
          switch (current.getKind()) {
            case kind::CONST_RATIONAL: {
              ...
              if (bv.toSignedInt() != constant.getNumerator()) {
                throw TypeCheckingException(
                    current.toExpr(),
                    std::string("Not enough bits for constant in intToBV: ") +
                        current.toString());
              }
            ...
            default:
              throw TypeCheckingException(
                  current.toExpr(),
                  std::string("Cannot translate const to BV: ") +
                      current.toString());
          }
        } else {
          throw TypeCheckingException(
